	...
	for index,value in pairs(linesArray) do
		if(value ~= "") then
			...
			if(type == "disk") then
				...
				listresult = listresult..""..deviceIcon.." ${color1}${font}${font FontAwesome:size=11} ${font}${font sans serif:size=9}${color}${execi 8 hddtemp "..path.." 2>/dev/null | rev | cut -d ' ' -f 1 | rev}"
